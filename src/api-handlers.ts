...
export class ApiHandlers {
  ...
  reviveElements(source) {
    ...
    const walk = (item, sink) => {
      ...
      if (typeof item === 'string' || typeof item === 'number') {
        ...
        if (text)
          sink.push(h('text', { content: text }));
        return;
      }
      if (typeof item !== 'object')
        return;
      if (typeof h.isElement === 'function' && h.isElement(item)) {
        sink.push(item);
        return;
      }
      ...
      if (type === 'text') {
        ...
        if (content === null || content === undefined || content === '')
          return;
        sink.push(h('text', Object.assign({}, attrs, { content: String(content) })));
        return;
      }
      const revivedChildren = [];
      for (const child of children)
        walk(child, revivedChildren);
      sink.push(h(type, Object.assign({}, attrs), revivedChildren));
    };
  ...
  async buildMediaMarkup(content, images, files) {
    ...
    const tempDir = path.join(this.ctx.baseDir, 'data', 'qq-chat', 'temp');
    const tempFiles = await this.safeReadDir(tempDir);
    for (const image of imageList) {
      const matched = tempFiles.filter((file) => file.includes(`temp_${image?.tempId}`));
      if (!matched.length)
        continue;
      const fileUrl = this.createMediaUrl(path.join(tempDir, matched[0]));
    ...
    for (const file of fileList) {
      ...
      if (type === 'audio')
        messageContent += h('audio', attrs).toString();
      else if (type === 'video')
        messageContent += h('video', attrs).toString();
      else
        messageContent += h('file', attrs).toString();
  ...
  async sendElementsToChannel(data) {
    try {
      ...
      const bot: any = this.ctx.bots.find((item) => item.selfId === selfId || item.user?.id === selfId);
      if (!bot)
  ...
  async recordStreamedBotMessage(selfId, bot, channelId, finalContent, streamMsgId) {
    ...
    await (this.fileManager as any).addMessageToFile(messageInfo);
  ...
  registerApiHandlers() {
    ...
    this.addListener('fetch-image', async (data) => {
      try {
        ...
        const dir = path.join(this.ctx.baseDir, 'data', 'qq-chat', 'persist-media', 'images');
        ...
        const toViteUrl = (targetPath) => {
          const relative = path.relative(mediaRoot, targetPath).replace(/\\/g, '/');
    ...
    this.addListener('get-bot-state', async (data) => {
      try {
        ...
        try {
          ...
          (this.logger as any)?.debug?.('查询群禁言状态失败（已忽略）:', error);
        }
        const botState = {
    ...
    this.addListener('send-message', async (data) => {
      try {
        ...
        const bot: any = this.ctx.bots.find((bot) => bot.selfId === data.selfId || bot.user?.id === data.selfId);
        if (!bot) {
        ...
        const parsedContent = h.parse(this.convertCmdInputTags(this.convertAtUserTags(messageContent)));
    ...
    this.addListener('get-member-role', async (data) => {
      try {
        const bot: any = this.ctx.bots.find((b) => b.selfId === data.selfId || b.user?.id === data.selfId);
        if (!bot)
          return { success: false, error: `未找到机器人 ${data.selfId}` };
    ...
    this.addListener('get-group-members', async (data) => {
      try {
        const bot: any = this.findGroupBot(data.selfId);
        if (!bot)
          return { success: false, error: `未找到机器人 ${data.selfId}` };
    ...
    this.addListener('upload-md-image', async (data) => {
      try {
        ...
        const assets: any = (this.ctx as any).assets;
        if (assets?.upload) {
        ...
        const tempDir = path.join(this.ctx.baseDir, 'data', 'qq-chat', 'temp');
        await fs.mkdir(tempDir, { recursive: true });
        const tempPath = path.join(tempDir, `md_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`);
        await fs.writeFile(tempPath, Buffer.from(matched[2], 'base64'));
    ...
    this.addListener('get-user-info', async (data) => {
      try {
        const bot: any = this.ctx.bots.find(b => b.selfId === data.selfId);
        if (!bot)
          return { success: false, error: '机器人不存在' };
    ...
    this.addListener('fetch-video-temp', async (data) => {
      try {
        ...
        const dir = path.join(this.ctx.baseDir, 'data', 'qq-chat', 'persist-media', 'media');
        await fs.mkdir(dir, { recursive: true });
        const hash = createHash('md5').update(data.url).digest('hex');
    ...
    this.addListener('run-command', async (data) => {
      const result = await runBridgedCommand({
        ctx: this.ctx,
        config: this.config,
        logger: this.logger,
        data
    ...
    this.addListener('simulate-message', async (data) => {
      const result = await runSimulatedMessage({
        ctx: this.ctx,
        config: this.config,
        logger: this.logger,
        data
    ...
    this.addListener('sandbox-reset', async (data) => resetSandboxStream(data));
    ...
    this.addListener('sandbox-send-message', async (data) => {
      try {
        ...
        if (!content.trim())
          return { success: false, error: '没有可发送的内容' };
        const result = await pushSandboxStream({
          ctx: this.ctx,
          config: this.config,
        ...
        return {
          success: true,
          content,
          elements: this.toWireElements(h.parse(content)),
    ...
    this.addListener('sandbox-clear', async (data) => clearSandboxSession(data));
  ...
  // ===== B 站 Wbi 签名 =====
  private static mixinKeyEncTab: number[] = [46, 47, 18, 2, 53, 8, 23, 32, 15, 50, 10, 31, 58, 3, 45, 35, 27, 43, 5, 49, 33, 9, 42, 19, 29, 28, 14, 39, 12, 38, 41, 13, 37, 48, 7, 16, 24, 55, 40, 61, 26, 17, 0, 1, 60, 51, 30, 4, 22, 25, 54, 21, 56, 59, 6, 63, 57, 62, 11, 36, 20, 34, 44, 52]
  ...
  async qqApiRequest(bot: any, method: string, path: string, data?: any): Promise<any> {
    const qqBot = bot;
  ...
  async resolveChatBackgroundUrl() {
    ...
    if (/^https?:\/\//i.test(source))
      return source;
    try {
      const filePath = path.isAbsolute(source) ? source : path.join(this.ctx.baseDir, source);
  ...
  async cleanupMediaCache() {
    const baseDir = path.join(this.ctx.baseDir, 'data', 'qq-chat', 'persist-media');
    await this.cleanupMediaCacheDir(path.join(baseDir, 'images'), 100);
    await this.cleanupMediaCacheDir(path.join(baseDir, 'audio'), 100);
