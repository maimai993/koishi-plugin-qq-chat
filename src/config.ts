 ...
 */
export const CONSOLE_AUTHORITY = 4
...
export const Config: Schema<Config> = Schema.intersect([
  Schema.object({
    ...
    ocrApiKey: Schema.string().default('helloworld').description('OCR 接口密钥（OCR.space 免费 key，默认 helloworld）。用于识别 B 站小程序卡片封面上的 UP 主，从而精确匹配视频。'),
  }).description('基础设置'),

  Schema.object({
    chatBackground: Schema.string().default('').description('聊天区背景图片：本地图片绝对路径（如 D:/pics/bg.jpg）或 http(s) 链接；留空使用纯色背景'),
    chatBackgroundBlur: Schema.number().default(0).description('背景图片模糊程度（px），0 表示不模糊').min(0).max(30).step(1),
    chatBackgroundDim: Schema.number().default(12).description('背景图片遮罩浓度（%），越大文字越清晰').min(0).max(90).step(2),
  }).description('聊天背景'),

  Schema.object({
    theme: Schema.union([
      Schema.const('koishi').description('跟随 Koishi 控制台'),
      Schema.const('system').description('跟随系统'),
  ...
  Schema.object({
    commandBridge: Schema.boolean().default(true).description('控制台发送的 /指令 由 Koishi 本地执行，拦截输出后再发到 QQ（关闭则原样发给 QQ）'),
    commandPrefix: Schema.string().default('/').description('本地指令前缀，例如 /'),
    commandAuthority: Schema.number().default(4).description('执行指令时使用的权限等级（4 = 管理员，保证多数指令可用）').min(0).max(5).step(1),
    commandTimeoutMs: Schema.number().default(50000).description('指令执行超时（毫秒）。绘图类指令耗时较长，建议 ≥50000；客户端 RPC 上限 60 秒').min(1000).max(55000).step(1000),
    commandMaxLength: Schema.number().default(1200).description('指令输出发送到 QQ 的最大长度').min(100).max(5000).step(100),
    commandEditRules: Schema.string().role('textarea').default('').description('输出编辑规则：每行一条「查找=>替换」，按顺序应用；用 \\n 表示换行'),
  }).description('控制台指令桥接'),
