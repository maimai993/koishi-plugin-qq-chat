...
export async function apply(ctx: Context, config: Config) {
  const pluginLogger = createPluginLogger(ctx.logger('qq-chat'), config)
  ...
  if (ctx.server) {
    const mediaRoot = path.join(ctx.baseDir, 'data', 'qq-chat')
    // 插件自带 QQ 表情素材目录（发布后位于 node_modules/koishi-plugin-qq-chat/qq_emoji）
    const emojiRoot = path.join(__dirname, '..', 'qq_emoji')
    ...
    const parseCookies = (header: unknown): Record<string, string> => {
    ...
    const authRequired = () => {
      if (config.loginRequired === false) return false
    ...
    const isConsoleAuthed = async (routerCtx: any): Promise<boolean> => {
      ...
      const database = (ctx as any).get?.('database')
    ...
    const needsAuth = (p: string) => p === '/qq-chat/background'
    ...
    ctx.server.use(async (routerCtx: any, next: any) => {
      ...
      if (p === '/qq-chat/fetch-image' || p.startsWith('/qq-chat/fetch-image?')) {
        try {
          ...
          const dir = path.join(mediaRoot, 'persist-media', 'images')
          ...
          if (!exists) {
            ...
            if (buffer.length) await fs.writeFile(filePath, buffer)
          }
      ...
      if (matchedWindow) {
        try {
          ...
          if (!rel) {
            const html = await fs.readFile(path.join(distDir, matchedWindow.html), 'utf8').catch(() => null)
            if (!html) {
              routerCtx.type = 'html'
            ...
            try {
              globalConfig = (ctx.console as any)?.createGlobal?.() || {}
            } catch {
              globalConfig = {}
            }
            if (!globalConfig.endpoint) {
              const consoleConfig: any = (ctx.console as any)?.config || {}
              globalConfig = {
                devMode: false,
      ...
      if (p === '/qq-chat/background') {
        try {
          ...
          const filePath = path.isAbsolute(source) ? source : path.join(ctx.baseDir, source)
          const stats = await fs.stat(filePath).catch(() => null)
          if (!stats || !stats.isFile()) {
            routerCtx.status = 404
            return
          }
          routerCtx.type = path.extname(filePath) || '.jpg'
      ...
      try {
        ...
        if (filePath.startsWith('/') || /^[A-Za-z]:[\\/]/.test(filePath)) {
          ...
          normalized = path.normalize(path.join(root, filePath))
        }
