...
/**
 * 拦截到的「内部 API / 原始 HTTP」请求 → 可渲染的输出片段。
 * 带 markdown 的（QQ 原生 markdown 消息，如 help-md 的菜单）保留成 markdown 元素，
 * 这样控制台里会按 markdown 渲染，而不是显示 ## / []() 这些原始语法。
 */
function outgoingFragment(type: string, ...args: any[]): any {
  for (const value of args) {
    if (!value || typeof value !== 'object') continue
    const md = value.markdown?.content ?? (typeof value.markdown === 'string' ? value.markdown : undefined)
    if (typeof md === 'string' && md.trim()) {
      try {
        return h('markdown', { content: md })
      } catch { /* 忽略，退回文本 */ }
    }
  }
  return describeOutgoing(type, ...args)
}

/**
 * 把「内部 API 调用 / 原始 HTTP 请求」描述成一段可读文本，
 * 这样沙盒里也能看到插件到底想发什么，并且可以改完再发。
 */
function describeOutgoing(type: string, ...args: any[]): any {
  const pick = (value: any): any => {
    if (value == null) return ''
    if (typeof value === 'string') return value
    if (typeof value !== 'object') return String(value)
    // QQ 官方 markdown / 按钮消息：把 markdown 正文掏出来
    const md = value.markdown?.content ?? value.markdown
    if (typeof md === 'string' && md.trim()) return md
    if (typeof value.content === 'string' && value.content.trim() && !value.file_type) return value.content
    // 文件 / 图片 / 语音：给出类型 + 地址
    if (value.file_type !== undefined || value.url) {
      const kind = ['图片', '视频', '语音', '文件'][Number(value.file_type)] || '文件'
      return `[${kind} ${value.url || value.file_info || ''}]`.trim()
    }
    if (Array.isArray(value)) return value.map(pick).join('')
    const text = value.content ?? value.text ?? value.title
    return typeof text === 'string' ? text : ''
  }
  const text = args.map(pick).filter((x) => String(x || '').trim()).join(' ')
  return text.trim() ? text : `[沙盒拦截] ${type}`
}

/**
 * 造一个「影子 bot」：原型继承真实 bot（适配器方法都在），但把**所有会真的碰到 QQ 的出口**都换成拦截器：
 *
 *  - `sendMessage` / `sendPrivateMessage` / `sendGroupMessage`
 *  - `internal.*`：QQ 官方 API（原生 markdown、上传文件、入群审批、群设置…）。
 *    之前只覆盖了 sendMessage，插件直接调 `session.bot.internal.sendMessage(...)`
 *    （如 koishi-plugin-help-md）会把消息真的发到 QQ —— 沙盒 / 指令桥接里必须堵死。
 *  - `http.post`：打到 /messages、/files、/stream_messages、/panels 这类写接口的请求
 *
 * 读操作（get/list/query/fetch 开头）放行，保证指令本身能正常取数据。
 */
function createShadowBot(realBot: any, capture: (channelId: any, content: any) => any): any {
  const shadow: any = Object.create(Object.getPrototypeOf(realBot))
  Object.assign(shadow, realBot)
  shadow.sendMessage = capture
  shadow.sendPrivateMessage = capture
  shadow.sendGroupMessage = capture
  const now = () => `qq-chat-sandbox-${Date.now()}`
  const realInternal = realBot?.internal
  if (realInternal) {
    shadow.internal = new Proxy(realInternal, {
      get(target, key, receiver) {
        const value = Reflect.get(target, key, receiver)
        if (typeof value !== 'function')
          return value
        const name = String(key)
        // 只读接口放行（取群成员、取群状态等），其余一律拦截
        if (/^(get|list|query|fetch|is|has|check|resolve|prepare)/i.test(name))
          return value.bind(target)
        return async (...args: any[]) => {
          try {
            capture(args[0], outgoingFragment(name, ...args.slice(1)))
          }
          catch { /* 忽略 */ }
          return { id: now(), message_id: now(), data: { id: now() } }
        }
      },
    })
  }
  const realHttp = realBot?.http
  if (realHttp && typeof realHttp.post === 'function') {
    shadow.http = new Proxy(realHttp, {
      get(target, key, receiver) {
        const value = Reflect.get(target, key, receiver)
        if (typeof value !== 'function')
          return value
        ...
        if (key !== 'post')
          return bound
        return async (url: any, data?: any, config?: any) => {
          if (!/\/(messages|files|stream_messages|panels)/i.test(String(url)))
            return bound(url, data, config)
          try {
            capture(undefined, outgoingFragment('http.post', data))
          }
          catch { /* 忽略 */ }
          return { data: { id: now(), message_id: now() }, status: 200 }
        }
      },
    })
  }
...
// 元素数组 → 纯文本
export function elementsToText(elements, maxLength) {
...
// 编辑规则：每行「查找=>替换」，按顺序应用
export function applyEditRules(text, rulesText, maxLength) {
  let out = String(text || '');
...
// 执行一条 Koishi 指令并捕获输出
export async function runBridgedCommand(options) {
  ...
  const push = (fragment) => {
    try {
      transcript.push(...h.normalize(fragment));
    }
    catch (error) {
  ...
  // 影子 bot：sendMessage / internal.* / http.post 全部拦截（否则插件直接调官方 API 会真的发到 QQ）
  session.bot = createShadowBot(bot, async (_channelId, content) => { push(content); return []; });
...
function createSandboxEntry(ctx, config, logger, data) {
  ...
  const push = (fragment) => {
    try {
      entry.transcript.push(...h.normalize(fragment));
    }
    catch { /* 忽略 */ }
  };
  session.send = async (fragment) => { push(fragment); return []; };
...
export async function runSandboxCommand(options) {
  ...
  try {
    ...
    if (result)
      entry.transcript.push(...h.normalize(result));
...
// 关闭沙盒：丢掉这个频道的会话上下文
export function clearSandboxSession(data) {
...
export async function runSimulatedMessage(options) {
  const { ctx, config, logger, data } = options || {};
  const selfId = String(data?.selfId || '');
  const channelId = String(data?.channelId || '');
  const isDirect = !!data?.isDirect;
  ...
  // 影子 bot：拦截所有会真的发到 QQ 的出口（sendMessage / internal.* / http.post）
  const captureSend = async (_channelId, fragment) => { push(fragment); return []; };
  const shadow = createShadowBot(bot, captureSend);
...
function getSandboxStream(ctx, selfId, channelId, isDirect) {
  ...
  const shadow = createShadowBot(realBot, capture);
  entry.bot = shadow;
...
export async function pushSandboxStream(options) {
  ...
  if (commandLine && content.startsWith(prefix)) {
    ...
    const timer = setTimeout(() => {
      if (settled)
        return;
      emit(h('text', { content: `指令 /${commandLine} 还在执行中，结果出来后会继续推给你…` }));
    }, timeoutMs);
    entry.session.execute(commandLine, true).then((result) => {
      ...
      clearTimeout(timer);
      if (!emitted)
        emit(h('text', { content: `执行失败：${error && error.message ? error.message : error}` }));
    });
    return { success: true };
  }
  // 普通消息 / 纯图片：真实派发进中间件
...
export function resetSandboxStream(data) {
  const key = `${String(data?.selfId || '')}:${String(data?.channelId || '')}`;
