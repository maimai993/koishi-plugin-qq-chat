 ...
 */
function outgoingFragment(type, ...args) {
    for (const value of args) {
        ...
        if (typeof md === 'string' && md.trim()) {
            try {
                return (0, koishi_1.h)('markdown', { content: md });
            }
            catch { /* 忽略，退回文本 */ }
        }
    }
 ...
 */
function describeOutgoing(type, ...args) {
    const pick = (value) => {
        if (value == null)
            return '';
        ...
        if (typeof md === 'string' && md.trim())
            return md;
        if (typeof value.content === 'string' && value.content.trim() && !value.file_type)
 ...
 */
function createShadowBot(realBot, capture) {
