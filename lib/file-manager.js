...
class FileManager {
     ...
     */
    static splitChannelKey(channelKey) {
     ...
     */
    async getChannelPreviews(channels, concurrency = 6) {
        await this.ensureMetadataLoaded();
        const result = {};
     ...
     */
    async markLatestBotMessageFailed(selfId, channelId, error) {
        const channelKey = `${selfId}:${channelId}`;
        const tempMessageId = this.peekLatestPendingBotMessageId(channelKey);
