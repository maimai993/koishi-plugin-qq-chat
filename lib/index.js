...
const mobile_api_1 = require("./mobile-api");
const notify_rules_1 = require("./notify-rules");
const read_state_1 = require("./read-state");
...
async function apply(ctx, config) {
    ...
    const readState = new read_state_1.ReadStateStore(ctx.baseDir, pluginLogger);
    // 推送设置（免打扰列表）：手机 App 的通知要遵守它
    const notifyRules = new notify_rules_1.NotifyRuleStore(ctx.baseDir, pluginLogger);
    const messageHandler = new message_handler_1.MessageHandler(ctx, config, fileManager, pluginLogger, readState);
    const apiHandlers = new api_handlers_1.ApiHandlers(ctx, config, fileManager, messageHandler, pluginLogger, readState);
    ...
    if (ctx.server) {
        ...
        const isConsoleCookieAuthed = async (routerCtx) => {
        ...
        const mobileApi = new mobile_api_1.MobileApi(ctx.baseDir, pluginLogger, {
            getRegistry: () => apiHandlers.getRegistry(),
            isConsoleAuthed: async (routerCtx) => isConsoleAuthed(routerCtx),
            hasConsoleSession: async (routerCtx) => isConsoleCookieAuthed(routerCtx)
        ...
        const isConsoleAuthed = async (routerCtx) => {
            if (!authRequired())
                return true;
            if (await isConsoleCookieAuthed(routerCtx))
                return true;
        ...
        const mobileApiRoute = async (routerCtx) => {
        ...
        if (typeof server.get === 'function') {
            ...
            if (typeof server.options === 'function')
                server.options('/qq-chat/api/:route', mobileApiRoute);
        }
        ...
        ctx.server.use(async (routerCtx, next) => {
            ...
            if (matchedWindow) {
                try {
                    ...
                    if (ext === '.webmanifest')
                        routerCtx.type = 'application/manifest+json';
                    else if (ext === '.js' && node_path_1.default.basename(filePath) === 'sw.js')
                        routerCtx.type = 'text/javascript';
                    else
                        routerCtx.type = ext;
