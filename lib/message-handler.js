...
class MessageHandler {
    constructor(ctx, config, fileManager, logger, readState) {
        this.ctx = ctx;
        this.config = config;
    ...
    describeSendError(error) {
        ...
        const fromList = Array.isArray(error?.errors)
            ? error.errors.map((item) => String(item?.message || item || '')).filter(Boolean).join('; ')
            : '';
        const raw = fromList
    ...
    extractMessageId(result) {
        ...
        const id = data?.id || data?.message_id || data?.msg_id || result.id || result.message_id;
        return id ? String(id) : '';
    }
    /**
     ...
     */
    wrapOutgoingSender(bot, invoke, resolveChannelId) {
        return async (...args) => {
    ...
    wrapBotSenders() {
        for (const bot of (this.ctx.bots || [])) {
            if (!bot || bot.platform !== 'qq' || bot.__qqChatSenderWrapped)
                continue;
            ...
            for (const name of ['sendMessage', 'sendPrivateMessage', 'sendGroupMessage']) {
                ...
                if (typeof original !== 'function')
                    continue;
                bot[name] = this.wrapOutgoingSender(bot, (args) => original.apply(bot, args), (args) => String(args[0] ?? ''));
            }
                ...
                if (internal && typeof internal === 'object') {
                    for (const key of Object.keys(internal)) {
                        ...
                        if (typeof original !== 'function')
                            continue;
                        // 读 / 配置类接口不碰
                        if (/^(get|list|query|fetch|is|has|check|resolve|prepare|set|delete|remove|approve|reject|acknowledge|update)/i.test(key))
                            continue;
                        if (!/^(send|post|upload|reply|stream|push|create|forward|put)/i.test(key))
                            continue;
                        internal[key] = this.wrapOutgoingSender(bot, (args) => original.apply(internal, args), (args) => {
                            const first = String(args[0] ?? '');
                            // 私聊类接口第一个参数是用户 openid，本地频道号要带 private: 前缀
                            if (/private|c2c|user|friend/i.test(key) && first && !first.startsWith('private:'))
            ...
            try {
                ...
                if (http && typeof http.post === 'function') {
                    ...
                    http.post = this.wrapOutgoingSender(bot, (args) => originalPost.apply(http, args), (args) => this.channelIdFromApiUrl(String(args[0] ?? '')));
                }
            }
            catch (error) {
    ...
    isAtBotMessage(session) {
        const selfId = session?.selfId;
        if (!selfId)
            return false;
        const hasAt = (elements) => {
            if (!Array.isArray(elements))
                return false;
            for (const el of elements) {
                if (!el)
                    continue;
                if (el.type === 'at' && el.attrs?.id === selfId)
                    return true;
                if (hasAt(el.children))
                    return true;
            }
            return false;
        };
        if (hasAt(session.elements)
            || hasAt(session.event?.message?.elements)
            || hasAt(session.event?.message?.children))
            return true;
        const content = String(session.content || session.event?.message?.content || '');
    ...
    stripLeadingAtSelf(content, selfId) {
        const text = String(content || '');
        if (!selfId || !/<at\b/i.test(text))
            return text;
    ...
    recordUserMessage(session, timestamp) {
        ...
        // 留下一条永远发不出去的「控制台」假消息
        if (session.qqChatSandbox || session.event?.qqChatSandbox || /^qq-chat:(?:sandbox|sim):/.test(String(session.messageId || '')))
            return;
    ...
    resolveBotName(session) {
        const cached = this.fileManager.getCachedBotInfo?.(session.selfId);
        const candidates = [session.bot?.user?.name, session.username, cached?.username];
        for (const name of candidates) {
            if (name && !this.isRawIdName(name))
                return name;
        }
        return `Bot-${session.selfId}`;
    }
    resolveBotAvatar(session) {
        const cached = this.fileManager.getCachedBotInfo?.(session.selfId);
        return session.bot?.user?.avatar || session.bot?.avatar || cached?.avatar || '';
    }
    ...
    async processUserMessage(session, timestamp) {
        try {
            ...
            if (session.quote) {
                const qUser = session.quote.user || {};
                let quoteUserId = qUser.userId || qUser.id || session.quote.userId || '';
                let quoteUserName = qUser.username || qUser.name || session.quote.username || '';
                ...
                if ((!quoteUserName || quoteUserName === 'unknown') && (session.quote.id || session.quote.messageId)) {
                    try {
                        const quoted = await this.fileManager.findChannelMessageById(session.selfId, session.channelId, session.quote.id || session.quote.messageId);
                        if (quoted) {
                            quoteUserId = quoteUserId || quoted.userId || '';
                            if (quoted.username && quoted.username !== 'unknown')
                                quoteUserName = quoted.username;
            ...
            if (this.readState) {
                try {
                    ...
                    this.logger.warn?.('累加未读状态失败:', error?.message || error);
                }
            }
