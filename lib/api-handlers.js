...
class ApiHandlers {
    constructor(ctx, config, fileManager, messageHandler, logger, readState) {
        this.ctx = ctx;
        this.config = config;
     ...
     */
    async markSendFailed(selfId, channelId, reason) {
    ...
    registerApiHandlers() {
        ...
        this.addListener('set-notify-muted', async (data) => {
            try {
                const muted = Array.isArray(data?.muted) ? data.muted.map((x) => String(x)) : [];
        ...
        this.addListener('mark-channel-read', async (data) => {
            try {
                if (!this.readState || !data?.selfId || !data?.channelId)
